...
def resolve_openclaw_home() -> Path:
    return Path(os.environ.get('OPENCLAW_HOME', str(Path.home() / '.openclaw')))


def load_config(path: str | None = None) -> dict:
    openclaw_home = resolve_openclaw_home()
    config_path = Path(path) if path else openclaw_home / 'orchestrator' / 'config.json'
    if config_path.exists():
        cfg = json.loads(config_path.read_text())
    else:
        cfg = {}

    db_path = cfg.get('database', {}).get('path', '${OPENCLAW_HOME}/orchestrator/orchestrator.db')
